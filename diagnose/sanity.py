...
def check_all_functioning_parameters(protocol,host,token):
    ...
    try:
        healthcheck_response= requests.get(f"{protocol}://{host}{constants.HEALTHCHECK_ENDPOINT}")
        ...
        print(f"Final Diagnosis: {total_checks - unsuccessful_checks} out of {total_checks} checks passed.")
    except Exception as e:
        print(constants.MESSAGE_SONARQUBE_SERVER_UNREACHABLE)
        print(constants.MESSAGE_SONARQUBE_SERVER_UNREACHABLE_HINT_MESSAGE)
        print(f"Diagnosis Failed. Only able to validate SonarQube User Token. Failed {unsuccessful_checks} and rest cannot be validated")
